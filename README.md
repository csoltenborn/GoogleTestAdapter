...
* Download and launch the VSIX installer from either the [Visual Studio Marketplace](https://marketplace.visualstudio.com/items?itemName=VisualCPPTeam.TestAdapterforGoogleTest) or [GitHub](https://github.com/Microsoft/TestAdapterForGoogleTest/releases)
...
* <a name="global_settings"></a>The *Test Adapter for Google Test* section of Visual Studio's *Tools/Options*. These options are referred to as *global options* in the following.
* <a name="toolbar"></a>The toolbar. The most important runtime options (i.e., *Parallel test execution*, *Break on failure*, *Catch exceptions*, and *Print test output*) can also be set via a toolbar; this is equivalent to setting the according options via *Tools/Options/Test Adapter for Google Test*.
...
* <a name="test_discovery_regex"></a>Switch on *Debug mode* at *Tools/Options/Google Test Adapter/General*, which will show on the test console whether your test executables are found by GTA. If they are not, you have two options:
  * Configure a *Test discovery regex* at the same place. 
...
The original Google Test Adapter is written and maintained by [Christian Soltenborn](https://github.com/csoltenborn) (main author) and [Jonas Gefele](https://github.com/jgefele) (regular contributor).
...
This particular fork of the project is maintained by the Visual C++ team at Microsoft. 
