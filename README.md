...
* Since leak detection is only perfomed when the executable is compiled with Debug configuration, the main method makes use of the `GTA_EXIT_CODE_SKIP` and `GTA_EXIT_CODE_OUTPUT_BEGIN` tokens to make sure the test is reported as skipped in that setting, and to additionally produce some meaningful output.
